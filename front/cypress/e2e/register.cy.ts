...
describe('Register spec', () => {
  ...
  it('should display an error if the required password field is missing', () => {
    // When
    // Remplissage form sans password 
    cy.get('input[formControlName=firstName]').type(user.firstName);
    cy.get('input[formControlName=lastName]').type(user.lastName);
  ...
  it('should display an error if the required email field is missing', () => {
    // When
    // Remplissage form sans password & submit
    cy.get('input[formControlName=firstName]').type(user.firstName);
    cy.get('input[formControlName=lastName]').type(user.lastName);
  ...
  it('should register, log in, verify user account details and delete it', () => {
    ...
    cy.get('.m3 mat-card-content p')
      .contains(`Name: ${user.firstName} ${user.lastName.toUpperCase()}`)
      .should('exist');
    cy.get('.m3 mat-card-content p')
